<x-layouts.app title="Edit Market Data | RSFLA">
    <div class="mx-auto max-w-3xl space-y-5">
        <div>
            <h1 class="font-rsfla-heading text-3xl font-bold text-[#424143]">Edit Market Data</h1>
            <p class="mt-2 text-sm text-[#424143]/65">{{ $property->name }}</p>
        </div>
        <form method="POST" enctype="multipart/form-data" action="{{ route('properties.market-data.update', [$property, $marketDataEntry]) }}" class="space-y-6 rounded-lg border border-[#424143]/10 bg-white p-6 shadow-sm">
            @csrf
            @method('PUT')
